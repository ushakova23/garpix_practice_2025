...
from wildbox_client import get_product_details, get_brand_details, get_warehouse_positions
...
def prepare_model_input(product_context, product_details, brand_details, city, search_query):
    ...
    temp_df['price_per_rating'] = (temp_df['price'] / temp_df['product_rating']).replace([np.inf, -np.inf], 0).fillna(0)
...
def calculate_ranking_factors(current_data):
    ...
    if current_data.get('delivery_efficiency_wh_avg_pos') is not None:
        delivery_score = min(100, max(0, 100 - (current_data.get('delivery_efficiency_wh_avg_pos', 0) / 100 * 100)))
        if delivery_score > 0:
...
def get_full_report(city: str, search_query: str, product_id: int):
    ...
    try:
        ...
        if not product_details or not has_essential_data:
            return None, f"Товар с артикулом {product_id} не найден или данные неполные.", None, None
            
    except Exception as e:
        return None, f"Ошибка API при получении данных товара {product_id}: {e}", None, None

    brand_id = product_details.get('brand', {}).get('id') if isinstance(product_details.get('brand'), dict) else None
    ...
    try:
        ...
        if positions_data:
            city_position_data = next((p for p in positions_data if p.get('city') == city), None)
...
st.markdown("**Анализ факторов ранжирования и автоматическая оптимизация для увеличения выручки**")

# Поля для ввода
with st.container():
    col1, col2, col3 = st.columns(3)
    with col1:
        product_id_input = st.text_input("Артикул товара (ID)", value="32395071", placeholder="Например: 16443420")
    with col2:
        search_query_input = st.text_input("Поисковый запрос", value="футболка женская белая", placeholder="Например: футболка женская белая")
    with col3:
        city_input = st.selectbox("Город", ["Москва", "Санкт-Петербург", "Новосибирск", "Хабаровск", "Екатеринбург", "Казань", "Краснодар"])
...
if st.button("🚀 Получить рекомендации", type="primary", use_container_width=True):
    if not product_id_input or not product_id_input.strip():
        ...
        with st.spinner("Анализирую данные с Wildbox и прогоняю через AI-модели..."):
            current_data, base_pos, base_proc, _ = get_full_report(city_input, search_query_input, int(product_id_input))
    ...
    if not current_data:
        st.error(base_pos)
            ...
            with col4:
                ...
                if price:
                    st.metric(
                        label="💵 Цена товара",
                        value=f"{price:,.0f}₽",
                        delta=f"-{current_data.get('discount', 0)}%" if current_data.get('discount') else None
                    )
                else:
                    st.metric(
                        label="💵 Цена",
            ...
            with col1:
                ...
                st.caption("Теоретическая важность факторов в алгоритме WB (по исследованиям)")
            ...
            with col2:
                ...
                if not factors_data:
                    ...
                    st.info("Возможные причины: товар новый, данные не полные, ограничения API")
                else:
                    for factor, value in factors_data.items():
                        ...
                        st.markdown(f"<div style='text-align: right; margin-top: -20px; margin-bottom: 10px;'><small>{value:.0f}%</small></div>", unsafe_allow_html=True)
            ...
            recommendations = generate_optimization_recommendations(current_data, base_pos, base_proc)
            ...
            if not rec_df.empty:
                ...
                st.info("🎉 Все основные факторы оптимизированы! Товар в хорошем состоянии.")
            ...
            if recommendations:
                ...
                with col2:
                    ...
                    total_cost = sum([int(rec['cost'].replace('₽', '').replace(',', '').split()[0]) for rec in recommendations if '₽' in rec['cost']])
                    if total_cost > 0:
                        st.write(f"• Общие затраты: ~{total_cost:,}₽")
                        st.write(f"• Средняя стоимость действия: {total_cost//len(recommendations):,}₽")
            else:
            ...
            with col1:
                ...
                promo_participation = st.slider("Участие в акциях:", 0, 100, max(20, int(factors_data.get('Акции', 20))), key="promo_slider")
                sales_volume = st.slider("Объем продаж:", 0, 100, max(45, int(factors_data.get('Продажи', 45))), key="sales_slider")
                conversion = st.slider("Конверсия:", 0, 100, max(60, int(factors_data.get('Конверсия', 60))), key="conversion_slider")
                product_rating = st.slider("Рейтинг товара:", 0, 100, max(75, int(factors_data.get('Рейтинг товара', 75))), key="rating_slider")
                card_completion = st.slider("Наполнение карточки:", 50, 100, 70, key="card_slider")
                price = st.slider("Цена товара:", 500, 5000, max(1200, int(current_data.get('price', 1200))), key="price_slider")
                
                # Дополнительные опции
                constructor_option = st.checkbox("Опция конструктора (+3%)", key="constructor_checkbox")
            ...
            with col2:
                ...
                try:
                    ...
                    st.metric(
                        "Новая позиция",
                        f"~ {int(new_pos_sim)}",
                        f"{position_change} мест" if position_change != 0 else "Без изменений",
                        delta_color="inverse" if position_change < 0 else "normal"
                    )
                    st.metric(
                        "Новая выручка",
                        f"~ {new_revenue_sim:,.0f}₽",
                        f"+{revenue_change:,.0f}₽" if revenue_change > 0 else "Без изменений"
                    )
                    
                    # Показываем общий эффект
                    if position_change != 0 or revenue_change > 0:
                        st.success(f"📈 Общий эффект: {abs(position_change)} позиций вверх, +{revenue_change:,.0f}₽ к выручке")
                    else:
