...
if not all([AUTH_TOKEN, COMPANY_ID, USER_ID, COOKIE_STRING]):
    raise ValueError(
        "Необходимо задать все переменные окружения в .env файле (AUTH_TOKEN, COMPANY_ID, USER_ID, COOKIE_STRING)"
    )
...
COOKIES = {
    cookie.split("=")[0]: cookie.split("=")[1] for cookie in COOKIE_STRING.split("; ")
}
...
def get_top_products(city: str, search_query: str, total_limit: int) -> list:
    ...
    base_params = {
        ...
        "extra_fields": "id,brand,auto_adv,cpm,position_dynamic,wh_avg_position,expected_position",
    }
    all_results = []
    page_size = 100
    offset = 0
    ...
    while len(all_results) < total_limit:
        ...
        try:
            response = requests.get(
                url, headers=HEADERS, cookies=COOKIES, params=params, timeout=45
            )
...
def get_warehouse_positions(product_id, search_query):
    ...
    try:
        ...
        print(
            f"[API Client] Получено записей: {len(data) if isinstance(data, list) else 'не список'}"
        )
