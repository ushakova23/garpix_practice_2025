...
class WBRankingModel:
    ...
    def _create_features_from_existing_data(self, df):
        ...
        df["price_per_rating"] = safe_divide(
            df.get("price", 0), df.get("product_rating", 1)
        )
        ...
        delivery_pos = df.get(
            "delivery_efficiency_wh_avg_pos", pd.Series([0] * len(df))
        ).fillna(0)
        ...
        try:
            df["price_category"] = pd.cut(
                price, bins=5, labels=["low", "below_avg", "avg", "above_avg", "high"]
            )
        ...
        try:
            df["stock_level"] = pd.cut(
                quantity, bins=4, labels=["out", "low", "medium", "high"]
            )
    ...
    def prepare_features_for_model(self, df):
        ...
        numeric_features = [
            ...
            "images_count",
            "cpm",
            "delivery_efficiency_wh_avg_pos",
            "is_discounted",
            "price_per_rating",
            "rating_weighted",
            "delivery_position_ratio",
            ...
            if cat_feature in feature_df.columns:
                if cat_feature not in self.label_encoders:
                    ...
                    try:
                        ...
                        most_frequent = (
                            feature_df[cat_feature].mode()[0]
                            if not feature_df[cat_feature].empty
                            else "unknown"
                        )
                        feature_df[cat_feature] = feature_df[cat_feature].fillna(
                            most_frequent
                        )
                        feature_df[f"{cat_feature}_encoded"] = self.label_encoders[
                            cat_feature
                        ].transform(feature_df[cat_feature].astype(str))

                numeric_features.append(f"{cat_feature}_encoded")

        # Выбираем только доступные признаки
        available_features = [
            col for col in numeric_features if col in feature_df.columns
        ]
    ...
    def train_models(self, df, test_size=0.2):
        ...
        valid_position_mask = (
            (y_position_raw > 0) & (~y_position_raw.isna()) & (y_position_raw != np.inf)
        )
        valid_revenue_mask = (
            (~y_revenue_raw.isna()) & (y_revenue_raw != np.inf) & (y_revenue_raw >= 0)
        )
        ...
        if valid_mask.sum() < 10:
            raise ValueError(
                f"Слишком мало хороших записей для обучения: {valid_mask.sum()}"
            )
        ...
        if "product_id" in processed_data_clean.columns:
            print(
                "Разделяем данные по артикулам (артикулы не пересекаются между выборками)..."
            )
            ...
            print(
                f"Обучающая выборка: {len(X_train)} записей ({len(train_articles)} артикулов)"
            )
            print(
                f"Тестовая выборка: {len(X_test)} записей ({len(test_articles)} артикулов)"
            )
    ...
    def save_models(
        self,
        position_path="models/position_model.pkl",
        revenue_path="models/proceeds_model.pkl",
    ...
    def load_models(
        self,
        position_path="position_model.pkl",
        revenue_path="proceeds_model.pkl",
